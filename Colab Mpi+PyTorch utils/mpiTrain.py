...
if my_rank !=0:
  name = list_names[my_rank-1]
  model = Net()
  _Optimizer = Optimizer(name)
  train_loader,test_loader = make_train_loader_test_loader()
  model_optimizer_dict = make_model_optimizer_dict(model,_Optimizer,train_loader,n_epochs)
comm.Barrier()
#прогоняем обучение

if my_rank !=0:
  epochesTest(name, model_optimizer_dict, n_epochs)
  # cохраняем красивые картинки 
  saveToHTML(name)
comm.Barrier()
